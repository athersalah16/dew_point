  ...
  return (
    <BaseSection title="projects" sectionID="projects">
      <div className="w-full flex  flex-col gap-7 px-5">
        ...
        <div className="w-full border-b border-b-blue-300   py-4 "/>

        {/* Display projects  */}
        <div className="w-full h-full px-3 gap-4 flex lg:flex-row flex-col lg:justify-between ">
          <div className="flex flex-col items-start lg:w-1/2 w-full min-h-full">
            ...
            <div className=" grid grid-cols-1 lg:grid-cols-1 gap-4 rounded-md p-4">
              {projects.map((project, index) => (
                <Project project={project} key={index} />
              ))}
            </div>
          </div>
          <div className=" lg:mt-20 flex justify-center  items-center h-96 w-full lg:w-[40%]  rounded-md">
            <Image
              src={"/services.png"}
              width={250}
              height={300}
